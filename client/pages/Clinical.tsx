...
const recentNotes = [
  {
    ...
    date: "2024-01-21",
    time: "10:30 AM",
    summary: "Patient showing good response to current medication regimen. Blood pressure stable.",
    status: "signed"
  },
  {
    id: "2",
    ...
    date: "2024-01-21",
    time: "09:15 AM",
    summary: "Post-operative assessment. Wound healing well, no signs of infection.",
    status: "pending"
  },
  {
    id: "3",
    ...
    date: "2024-01-20",
    time: "02:45 PM",
    summary: "Routine prenatal checkup. Fetal development normal, maternal vitals stable.",
    status: "signed"
  }
];
    ...
    <BaseLayout title="Clinical">
      ...
      <div className="grid grid-cols-12 gap-6">
          ...
          <Card className="shadow-lg">
            <CardHeader>
              <CardTitle className="text-lg font-bold text-gray-700">
                Lab Results <span className="text-gray-400 font-normal">({labResults.length} pending review)</span>
              </CardTitle>
            </CardHeader>
            <CardContent>
                    ...
                    {labResults.map((result) => (
                      <tr key={result.id} className="border-b border-gray-100 hover:bg-gray-50">
                        <td className="py-3 px-4">
                          ...
                          <p className="text-sm text-gray-500">{result.physician}</p>
                        </td>
                        <td className="py-3 px-4 text-sm text-gray-900">{result.test}</td>
                        <td className="py-3 px-4">
                          <Badge className={resultColors[result.result as keyof typeof resultColors]}>
                            {result.result}
                          </Badge>
                        </td>
                        <td className="py-3 px-4">
                          <p className="text-sm text-gray-900">{result.value}</p>
        ...
        <div className="col-span-4 space-y-6">
          ...
          <Card className="shadow-lg">
            <CardHeader>
              <CardTitle className="text-lg font-bold text-gray-700">Latest Vital Signs</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              {vitalSigns.map((vitals, index) => (
                <div key={index} className="border border-gray-200 rounded-lg p-3">
                  <div className="flex items-center justify-between mb-3">
                    <h4 className="font-medium text-gray-900">{vitals.patient}</h4>
                    <span className="text-xs text-gray-500">{vitals.timestamp}</span>
                  </div>
          ...
          <Card className="shadow-lg">
            <CardHeader>
              <CardTitle className="text-lg font-bold text-gray-700">Quick Note</CardTitle>
            </CardHeader>
            <CardContent className="space-y-4">
              <div>
              ...
              <div className="flex space-x-2">
                ...
                <Button className="flex-1 bg-violet-600 hover:bg-violet-700">Sign & Submit</Button>
              </div>
            </CardContent>
          </Card>
        </div>
